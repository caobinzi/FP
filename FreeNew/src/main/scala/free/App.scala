...
object FreeApp extends App {
  ...
  def prg[F[_]](implicit I: Interact -~> F, C: Crud -~> F, L: Log -~> F): Free[F, Boolean] = {
    import Interact._
    import Crud._
    import Log._
  ...
  val program: Free[PRG, Boolean] = prg[PRG]
  ...
  val result0: Result[Boolean] = program.foldMap(interpreter)
